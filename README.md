...
* `Babel-polyfill` makes our app compatible with old browsers. Install via npm and add it as an entry point in the webpack config file.
...
* When working with a database, such as Firebase, the action generators will have to be modified to dispatch a function instead of an object. Within that function, a call will be made to the database to perform the necessary async operation, then when the operation is complete, an object can then be dispatched to the store to update state, to reflect the database change.
...
## AUTOMATED TESTING
* React automated testing is done with the Jest library, which is the most suitable for React applications. Test files use the test.js file extension, which enables Jest to locate these files appropriately. Test cases are defined using the `test()` global method, passing in a string as the first argument, and a callback function as the second argument. To make the test asynchronous, `done` is passed as an argument into the callback, and called when the async operation is completed.
...
* `redux-mock-store` is a library that provides a mock store for testing dispatched actions.
### Snapshot Testing
...
## FIREBASE
* Firebase is a no-SQL database which also has authentication functionality. The client is installed as an NPM library, and the configuration object is gotten after creating a project on a Firebase account.
* `firebase.initializeApp()` sets up the connection taking in the config object as an argument. 
* `database()` connects to the database on every call. There is no rigid schema, any type or structure of data can be saved into the database. However, firebase does not store arrays as lists. It stores individual lists as properties on an object, instead of lists in an array. For an individual property, the key is an auto-generated ID from the database, and the value would be the list itself. `.push()` is used to add properties to the object. When retrieved from the database, the object of objects would be converted to an array using the `forEach()` method available on snapshot.
* `.ref()` points to a particular group of data (synonymous to collections in MongoDB or tables in MySQL) stored as a collection. It takes the field that needs to be updated as an argument.
* `.set()` is called on ref() and inserts data into the database. It overrides any data that exists in the referenced location. It returns a promise which is resolved with a then() and catch() call.
* `.remove()` is also called on ref() and returns a promise, removing the specified data passed into the ref(). Passing in 'null' as a value to .set() also works to remove a property from the data.
* `.update()` takes in fields that need to be updated in the reference and updates.
* To retrieve from the database, 
    * `.once()` retrieves the information only once. The promise returned is resolved with snapshot passed into the then() call. `snapshot.val()` gives the actual data required. `snapshot.key` provides the key of that snapshot or childsnapshot, as the case may be, which is the ID. 
    * `.on()` subscribes to the database and fetches all subsequent changes made to the database real-time. It however uses only callbacks to resolve the data that is retrieved or handle the error that is thrown. An event is passed in as the first argument, such as `value `, to retrieve the value of the object, or `child_removed` which returns a nested object that was removed from the object. Other events are `child_added`, `child_updated`.
    * `.off()` unsubscribes from the database and stops data from being fetched realtime. It takes in the callback function specified in the initial subscription as it's argument.
### Connecting Redux to Firebase
* React components cannot dispatch functions, only objects by default. A middleware `redux-thunk` gives the functionality to be able to do this. To use middleware in Redux, the `applyMiddleware()` function needs to imported and used. A component will therefore be able to dispatch a function to the Redux store, which executes it, communicating with Firebase.

## AUTHENTICATION WITH FIREBASE
* To set up authentication, the provider needs to be selected in Firebase, then a new instance of that provider created in the firebase config file.
* Data validation is done by configuring rules in the database account.

## CREATE-REACT-APP
* It provides all the necessary dependencies needed to run a react app just by running a single command. A directory with all files needed is created. It utilises webpack and babel under the hood, no configuration needed.

## REACT HOOKS
* React hooks are functions that give functional-stateless-components access to state and life-cycle methods. There are inbuilt hooks and user-defined hooks can also be created.
* `useState()` provides access to state inside of a functional component. It improves on state usage, because state no longer has to be defined only as an object. It can be any other data type. It takes in the initialising value of state as an argument. It returns an array containing two items: the current state value, and a function that is used to update the state. Multiple calls to useState() are required to track different pieces of state data, in contrast with the old pattern using setState(). This is because changing the state variable will mutate everything on it. Therefore, it is better to make use of reducers to manipulate complex state objects.
* `useEffect()` enables the usage of life-cycle methods in functional components. It performs the function of componentDidMount and componentDidUpdate. It is called with a callback function which runs once when a component is first rendered, then fires again every time state or props changes. The second argument is an array (*an array of dependencies*) which is used to specify which items in state the callback should run for. This makes for efficiency so that the callback doesn't run when state is changed for items that we do not want to manipulate within the function. If the array is defined as empty, the callback will run only once on first rendering, and will not run on subsequent re-renders. This is particularly important for components that get mounted and unmounted frequently. useEffect() can also be called multiple times for different use cases, unlike componentDidMount which is called only once for all actions that need to be done. To simulate componentWillUnmount(), useEffect() returns a function within the callback function. This function runs when the component is being removed. This is called *cleaning up effects*. It is very useful for cleaning up eventlisteners when a component is being unmounted.
* `useReducer()` provides the same functionality that Redux does. It takes in a reducer function which is defined the way it would with Redux. It also takes in the current state as the second argument. It returns the state and a dispatch function. This provides the same functionality that the connect() function does in Redux by making state and the dispatch function available to the component. It is a better alternative to useState() when managing state involves using complex logic with objects and arrays, rather than primitive types.
* `useContext()` and `React.createContext()` create the same functionality provided by Redux to connect independent components with state and dispatch. React.createContext() is called (saved in a variable SharedContext) and made available to all components that need to share data. The parent component will wrap the child components that need to share its data with a component defined on SharedContext.Provider. Thus, there would be no need to pass down any props. The components accessing the data would simply call useContext(SharedContext) to access the state or dispatch function as needed.
* `fragments` are introduced in React to eliminate the unnecessary use of div elements as a root element. A fragment - `<> </>`  - wraps JSX without the need to use a div.
### Custom Hooks
* We can write custom hooks which make use of other built-in hooks, to reuse code between components. It's a convention to start the function names with the word *use*, to distinguish it as a React hook from other regular functions.
